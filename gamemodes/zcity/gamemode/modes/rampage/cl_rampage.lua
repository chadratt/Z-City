...
local song
local songfade = 0
net.Receive("rampage_start", function()
	surface.PlaySound("snd_jack_hmcd_panic.mp3") 

	timer.Simple(3, function()
		sound.PlayFile( "sound/rampage/readyornottheme.mp3", "mono noblock", function( station )
...
local teams = {
	...
	[1] = {
		objective = "Theres someone going around and killing everybody... i have to do something about it",
		name = "a Civilian",
...
function MODE:RenderScreenspaceEffects()
	zb.RemoveFade()
	if zb.ROUND_START + 130 < CurTime() then
...
function MODE:HUDPaint()
	...
	if zb.ROUND_START + 8.5 > CurTime() then
		...
		draw.SimpleText("Rampage", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0, 162, 255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
local CreateEndMenu
net.Receive("rampage_end", function() CreateEndMenu(net.ReadBool()) end)
