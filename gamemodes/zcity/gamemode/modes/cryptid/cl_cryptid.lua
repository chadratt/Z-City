...
net.Receive("cryptid_start", function()


	cryptidFogActive = true
	fogStartTime = CurTime()


	surface.PlaySound("cryptidmusic/cryptidTheme.mp3")


	timer.Simple(3, function()
		sound.PlayFile("cryptidmusic/cryptidTheme.mp3", "mono noblock", function(station)
...
local function FogEnabled()
	return cryptidFogActive and fogStartTime > 0
end

local function GetFogLerp()
	return math.Clamp((CurTime() - fogStartTime) / 60, 0, 1)
end
...
local teams = {
	[0] = {
		objective = "Blend in, after a minute you will transform into a blood thirsty monster.",
		name = "The Monster",
		color1 = Color(68, 10, 255),
		color2 = Color(68, 10, 255)
	},
	[1] = {
		objective = "You feel a eery presence around, watch out and prepare.",
		name = "A survivor",
...
function MODE:RenderScreenspaceEffects()
	...
	if cryptidFogActive and fogStartTime > 0 then
		local t = math.Clamp((CurTime() - fogStartTime) / 60, 0, 1)

		DrawColorModify({
			["$pp_colour_contrast"] = 2 - (t * 0.7),
			["$pp_colour_colour"] = 1 - (t * 0.8)
		})
	else
...
function MODE:HUDPaint()
	if zb.ROUND_START + 90 > CurTime() then
		posadd = Lerp(FrameTime() * 5, posadd or 0,
			zb.ROUND_START + 7.3 < CurTime() and 0 or -sw * 0.4)

		local color = Color(255 * -math.sin(CurTime() * 3), 25, 255 * math.sin(CurTime() * 3))

		draw.SimpleText(
			"The Creature Will reveal itself in: " ..
			string.FormattedTime(zb.ROUND_START + 90 - CurTime(), "%02i:%02i"),
			"ZB_HomicideMedium",
			sw * 0.02 + posadd,
			sh * 0.95,
			Color(0, 0, 0),
	...
	if zb.ROUND_START + 8.5 > CurTime() then
		...
		draw.SimpleText("The Invader", "ZB_HomicideMediumLarge",
			sw * 0.5, sh * 0.1,
		...
		draw.SimpleText("You are " .. Rolename,
