...
util.AddNetworkString("routekanal_start")
...
function MODE:GiveEquipment()
	timer.Simple(0.1, function()
		...
		for _, ply in RandomPairs(players_alive) do
			...
			if ply:Team() == 1 then
				if subgunner > 0 and not ply.subClass then
					subgunner = subgunner - 1
					ply.subClass = "subgunner"
					if not leader then
						ply.leader = true
						ply:SetNWString("PlayerRole", "subgunner")
						leader = true
					end
				end
			...
			ply:SetPlayerClass(ply:Team() == 1 and "metrocoproutekanal" or "Refugeeroutekanal")
...
util.AddNetworkString("routekanal_roundend")
