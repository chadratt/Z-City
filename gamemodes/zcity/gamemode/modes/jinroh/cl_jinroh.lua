...
net.Receive("jinroh_start", function()
	surface.PlaySound("jinrohgamemode/intro.mp3") 

	timer.Simple(3, function()
		sound.PlayFile( "sound/jinrohgamemode/pertubator.mp3", "mono noblock", function( station )
...
local teams = {
	[0] = {
		objective = "Eliminate the Sect.",
		name = "a Member of the Wolf Brigade",
		color1 = Color(68, 10, 255),
		color2 = Color(68, 10, 255)
	},
	[1] = {
		objective = "Defend against the wolf brigade",
		name = "a Member of the Sect",
...
function MODE:HUDPaint()
	if zb.ROUND_START + 30 > CurTime() then
		...
		draw.SimpleText( "The Wolf Brigade will arrive in: "..string.FormattedTime(zb.ROUND_START + 30 - CurTime(), "%02i:%02i"	), "ZB_HomicideMedium", sw * 0.02 + posadd, sh * 0.95, Color(0,0,0), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		draw.SimpleText( "The Wolf Brigade will arrive in: "..string.FormattedTime(zb.ROUND_START + 30 - CurTime(), "%02i:%02i"	), "ZB_HomicideMedium", (sw * 0.02) - 2 + posadd, (sh * 0.95) - 2, color, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		local fade = math.Clamp(zb.ROUND_START + 7.5 - CurTime(), 0, 1)
		surface.SetDrawColor(0, 0, 0, 255 * fade)
		surface.DrawRect(-1, -1, ScrW() + 1, ScrH() + 1)
	end
	...
	if zb.ROUND_START + 8.5 > CurTime() then
		...
		draw.SimpleText("Jin Roh", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0, 162, 255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
local CreateEndMenu
net.Receive("jinroh_end", function() CreateEndMenu(net.ReadBool()) end)
