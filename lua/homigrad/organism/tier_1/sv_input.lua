...
hook.Add("EntityTakeDamage", "homigrad-damage", function(ent, dmgInfo)
	...
	if not org.superfighter then
		...
		local bleed_add = dmgBlood * bleedMul
	...
	if dmgInfo:IsDamageType(DMG_BULLET + DMG_BUCKSHOT + DMG_BLAST + DMG_SLASH + DMG_CLUB + DMG_GENERIC) then
		...
		if ply then
			...
			if ply.AddForceRag[bone][2] and ply.AddForceRag[bone][2]:Length() > 4500 then
	...
	if org.isPly then
		...
		if (not ply:Alive() or not org.alive) and (math.Round(ply:GetInfoNum("hg_deathfadeout", 1)) == 1) then
			if org.skull == 1 then
...
local function velocityDamage(ent, data)
	...
	if not org.superfighter then
		if hitgroup == HITGROUP_LEFTLEG and (dmg * 3 > 0.25) then hg.organism.input_list.llegup(org, bone, dmg * 1 * math.Rand(1, 2), dmgInfo) end
		...
		if hitgroup == HITGROUP_STOMACH and physAng:Forward():Dot(data.HitNormal) > 0.6 then hg.organism.input_list.spine1(org, bone, dmg * (math.random(3) > 1 and 1 or 0) * 3, dmgInfo) end
...
hg.legJointBones = legJointBones

function hg.FreeBrokenLegBones(rag, key)
	if !IsValid(rag) or !rag:IsRagdoll() then return end

	local bones = legJointBones[key]
	if !bones then return end

	local thighid = rag:LookupBone(bones[1])
	local calfid = rag:LookupBone(bones[2])
	if !thighid or thighid < 0 or !calfid or calfid < 0 then return end

	local thighphys = rag:TranslateBoneToPhysBone(thighid)
	local calfphys = rag:TranslateBoneToPhysBone(calfid)
	if !thighphys or thighphys < 0 or !calfphys or calfphys < 0 then return end

	rag.brokenLegBones = rag.brokenLegBones or {}
	if rag.brokenLegBones[calfphys] then return end

	local pthigh = rag:GetPhysicsObjectNum(thighphys)
	local pcalf = rag:GetPhysicsObjectNum(calfphys)
	if !IsValid(pthigh) or !IsValid(pcalf) then return end

	rag.brokenLegBones[calfphys] = true

	rag:RemoveInternalConstraint(calfphys)

	local lpos = WorldToLocal(pcalf:GetPos(), angle_zero, pthigh:GetPos(), pthigh:GetAngles())

	constraint.AdvBallsocket(rag, rag, thighphys, calfphys, lpos, nil, 0, 0, -180, -180, -180, 180, 180, 180, 0, 0, 0, 0, 0)
