...
if CLIENT then
	...
	local function CreateNotification(msg, showTimer, clr, shaky)
		...
		table.insert(hg.notifications, {msg, (showTimer or defaultShowTimer), clr or Color(255, 255, 255, 255), shaky and true or false})
	end
	...
	net.Receive("HGNotificate",function()
		...
		CreateNotification(msg, showtime, clr, shaky)
	end)
	...
	local maxtimefade = 1
	local oldclick = 0
	local NOTIFY_BOX_COLOR = Color(0, 0, 0, 255 * 0.7)
	local NOTIFY_BOX_PADDING_X = 14
	local NOTIFY_BOX_PADDING_Y = 10
	local NOTIFY_BOX_RADIUS = 6
	local NOTIFY_FADE_IN_TIME = 0.5
	local NOTIFY_SLIDE_DISTANCE = 8
	local function DrawNotificationBox(x, y, w, h, alphaMul)
		local col = ColorAlpha(NOTIFY_BOX_COLOR, NOTIFY_BOX_COLOR.a * alphaMul)
		draw.RoundedBox(NOTIFY_BOX_RADIUS, x, y, w, h, col)
	end
	sound.Add({
		name = "peepsnd",
		channel = CHAN_AUTO,
	...
	local function NotificationsDraw()
		...
		local tbl = hg.currentNotification
		if tbl and istable(tbl) and not table.IsEmpty(tbl) then
			local msg, time, timeshow, clr, forceShaky = tbl[1], tbl[2], tbl[3], tbl[4], tbl[5]
			local shakeAmt = forceShaky and 3 or 0
			local mul = ((org.brain > 0.1 or org.pulse < 50) and 3 or 1)
			...
			if (time + time_to_read + wait > time_spent) then
				...
				local isCalm = org.pain <= 10
				local entrance = 1
				if isCalm then
					entrance = math.Clamp((time_spent - time) / NOTIFY_FADE_IN_TIME, 0, 1)
					entrance = 1 - (1 - entrance) ^ 3
				end
				local slideOffset = (1 - entrance) * NOTIFY_SLIDE_DISTANCE
				...
				elseif lply.PlayerClassName == "furry" then
					local x, y = ScrW() / 2 - txtw / 2 + math.Rand(-shakeAmt, org.pain > 10 and org.pain / 10 or shakeAmt) + math.Rand(0, (255 - clr.g) / 255 * 2), ScrH() - ScrH() / 6 + math.Rand(-shakeAmt, org.pain > 10 and org.pain / 10 or shakeAmt) + math.Rand(0, (255 - clr.g) / 255 * 2) + slideOffset
					local boxAlpha = ( col.a / 255 ) * entrance
					DrawNotificationBox( x - NOTIFY_BOX_PADDING_X, y - ( txth + NOTIFY_BOX_PADDING_Y * 2 ) / 2, txtw + NOTIFY_BOX_PADDING_X * 2, txth + NOTIFY_BOX_PADDING_Y * 2, boxAlpha )
					draw.SimpleText(last_message or txt, "ZB_ProotOSMedium", x + 2, y + 2, ColorAlpha(color_black, col.a * entrance), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
					draw.SimpleText(last_message or txt, "ZB_ProotOSMedium", x, y, ColorAlpha(bluewhite, col.a * entrance), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
				else
					local x, y = ScrW() / 2 - txtw / 2 + math.Rand(-shakeAmt, org.pain > 10 and org.pain / 10 or shakeAmt) + math.Rand(0, (255 - clr.g) / 255 * 2), ScrH() - ScrH() / 6 + math.Rand(-shakeAmt, org.pain > 10 and org.pain / 10 or shakeAmt) + math.Rand(0, (255 - clr.g) / 255 * 2) + slideOffset
					local boxAlpha = ( col.a / 255 ) * entrance
					DrawNotificationBox( x - NOTIFY_BOX_PADDING_X, y - ( txth + NOTIFY_BOX_PADDING_Y * 2 ) / 2, txtw + NOTIFY_BOX_PADDING_X * 2, txth + NOTIFY_BOX_PADDING_Y * 2, boxAlpha )
					draw.SimpleTextOutlined(last_message or txt, font, x, y, ColorAlpha(col, col.a * entrance), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1.5, ColorAlpha(colBrown, colBrown.a * entrance))
				end
			else
				local tbl = hg.currentNotification
				local clr = tbl and tbl[4] and IsColor(tbl[4]) and tbl[4] or Color(255, 255, 255, 255)
