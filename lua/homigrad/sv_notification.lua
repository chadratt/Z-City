...
local function AdminNotify(ply, msg, clr, shaky)
    if not IsValid(ply) or not ply:IsPlayer() then return false end
    if not msg or not isstring(msg) or msg == "" then return false end
    clr = clr or color_white
    local clr2 = Color(clr.r, clr.g, clr.b, 255)
    net.Start("HGNotificate")
    net.WriteString(msg)
    net.WriteColor(clr2)
    net.WriteBool(shaky and true or false)
    net.Send(ply)
    return true
end
hg.AdminNotify = AdminNotify
